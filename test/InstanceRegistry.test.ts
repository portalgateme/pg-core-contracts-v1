...
import {
  deployInstances,
  formatToContractInput,
} from "../utils/deployInstances";
...
describe("InstanceRegistry", async function () {
  ...
  describe("constructor", async function () {
    it("should set governance", async function () {
      const { deployer } = await setup();
      expect(await deployer["InstanceRegistry"].governance()).to.be.equal(
        deployer.address
      );
    });
  });
  ...
  describe("initInstances", async function () {
    it("should set instances", async function () {
      ...
      expect(
        cast(await InstanceRegistry.instances(formattedInstance.addr))
      ).to.deep.equal(formattedInstance.instance);
    });
    ...
    it("should emit event", async function () {
      ...
      await expect(
        deployer["InstanceRegistry"].initInstances(formattedInstances)
      )
    ...
    it("should revert if status is not disabled", async function () {
      ...
      await expect(
        InstanceRegistry.updateInstance(formattedInstance)
      ).to.be.revertedWith("Use removeInstance() for remove");
    ...
    describe("removeInstance", async function () {
      it("should emit event", async function () {
        ...
        await expect(InstanceRegistry.removeInstance(0))
          .to.emit(InstanceRegistry, "InstanceStateUpdated")
      ...
      it("should update state", async function () {
        ...
        await InstanceRegistry.removeInstance(0);

        expect(
          cast(await InstanceRegistry.instances(formattedInstance.addr)).state
        ).to.equal(0);
      });
    });
    ...
    describe("getAllInstances", async function () {
      it("should return all instances", async function () {
        ...
        expect(cast(getInstances)).to.deep.equal(formattedInstances);
      });
    });
    ...
    describe("getAllInstanceAddresses", async function () {
      it("should return all instance addresses", async function () {
        ...
        expect(addresses).to.deep.equal(
          formattedInstances.map((instance) => instance.addr)
        );
      });
    });
    ...
    describe("getPoolToken", async function () {
      it("should return pool token", async function () {
        ...
        expect(
          await InstanceRegistry.getPoolToken(formattedInstance.addr)
        ).to.equal(formattedInstance.instance.token);
      });
    });

    describe("setPGRouter", async function () {
      it("should set PGRouter", async function () {
        const { InstanceRegistry, deployer, instances, PGRouter } =
          await setup();
      ...
      it("should revert if not governance", async function () {
        const { InstanceRegistry, deployer, instances, PGRouter, users } =
          await setup();

        await expect(
          users[0]["InstanceRegistry"].setPGRouter(PGRouter.address)
    ...
    describe("updateInstanceState", async function () {
      it("should update state", async function () {
        ...
        await InstanceRegistry.updateInstanceState(formattedInstance.addr, 2);
      ...
      it("should emit event", async function () {
        ...
        await expect(
          InstanceRegistry.updateInstanceState(formattedInstance.addr, 2)
        )
          .to.emit(InstanceRegistry, "InstanceStateUpdated")
          .withArgs(formattedInstance.addr, 2);
      });
    });
  });
});
